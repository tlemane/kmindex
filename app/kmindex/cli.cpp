...
namespace kmq {
  ...
  kmq_cli::kmq_cli(const std::string& name, const std::string& desc, const std::string& version)
  {
    ...
    m_kmq_dump_opt = std::make_shared<struct kmq_dump_options>(kmq_dump_options{});
    ...
    kmq_dump_cli(m_cli_parser, m_kmq_dump_opt);
  }
  ...
  std::tuple<kmq_commands, kmq_options_t> kmq_cli::parse(int argc, char* argv[])
  {
    ...
    else if (m_cli_parser->is("dump"))
      return std::make_tuple(kmq_commands::kmq_dump, m_kmq_dump_opt);
    else
      exit(EXIT_FAILURE);
  }
}
